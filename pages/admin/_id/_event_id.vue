<template>
  <div class="bg-grey-light">
    <div class="flex w-full border-b py-4 items-center bg-white">
      <button class="flex items-center uppercase px-4 py-3 hover:bg-grey-light mx-4" @click="back">Go Back</button>
      <div class="text-grey-darkest text-xl px-4 border-l">Edit "{{name}}"</div>
    </div>
    <div class="flex pt-8 px-5">
      ...
      <form @submit.prevent="save" class="w-2/3 p-5 border bg-white">
        ...
        <div class="flex flex-col text-left my-3">
          ...
          <input name="eventId" v-model="id" type="text" required  class="p-3 border-b mt-2" readonly aria-readonly="true"/>
        </div>
        <div class="flex flex-col text-left my-3">
...
export default {
  ...
  methods: {
    async save() {
      ...
      const response = await this.$axios.post(`/api/updateEvent?id=${this.id}`, {
        payload
      });
      ...
      if (response.error || response.data.error) {
        ...
        this.$toast.success('Event details updated succeeded. Redirecting back to event list', {
          duration: 2000,
        });
